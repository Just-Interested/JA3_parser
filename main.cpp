...
void update(JA3_parser &parser, Config &config){
    std::ofstream out;
    std::string output_file_name = config.GetOutputFileName();
    ...
    while(1){
        ...
        out.close();
        std::this_thread::sleep_for(std::chrono::seconds(config.GetCheckInterval()));
    }
}
...
int parse(){
    ...
    cfg.ReadConfig();
    JA3_parser parser;
    parser.setDirToParse(cfg.GetPcapFilesDir());
...
void printUsage(){
    std::cout << "Program checks config file ./ja3_parser.conf or /etc/ja3_parser.conf\n"
            ...
            << "Options: \n"
            << "-h              prints this help\n"
            << "--nodaemon      run as usual program"
            << std::endl;
}

int running_daemon = 1;
